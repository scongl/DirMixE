...
class ImbalanceCIFAR100DataLoader(DataLoader):
    ...
    def get_few_shot_classes(self, eta):
...
class  DirichletImbalanceCIFAR100DataLoader(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.01, test_imb_factor=0, reverse=False):
        ...
        if training:
            ...
            if test_imb_factor!=0:
                dataset = DirichletImbalanceCIFAR100(data_dir, train=False, download=True, transform=train_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse)
            else:
                dataset = test_dataset
            val_dataset = None
