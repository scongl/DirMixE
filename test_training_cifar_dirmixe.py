...
from data_loader.imbalance_cifar import DirichletImbalanceCIFAR100, DirichletImbalanceCIFAR10
...
class DirichletImbalanceCIFAR100DataLoader(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.01, test_imb_factor=0, reverse=False):
        ...
        if training:
            dataset = DirichletImbalanceCIFAR100(data_dir, train=True, download=True, transform=train_trsfm, imb_type=imb_type, imb_factor=imb_factor)
            val_dataset = test_dataset
        else: 
            dataset = DirichletImbalanceCIFAR100(data_dir, train=True, download=True, transform=train_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
            train_dataset = DirichletImbalanceCIFAR100(data_dir, train=False, download=True, transform= TwoCropsTransform(train_trsfm), imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
            val_dataset = DirichletImbalanceCIFAR100(data_dir, train=False, download=True, transform=test_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
...
class DirichletImbalanceCIFAR10DataLoader(DataLoader):
    """
    Imbalance Cifar100 Data Loader
    """
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.01, test_imb_factor=0, reverse=False):
        ...
        test_dataset = datasets.CIFAR10(data_dir, train=False, download=True, transform=test_trsfm) # test set
        
        if training:
            dataset = DirichletImbalanceCIFAR10(data_dir, train=True, download=True, transform=train_trsfm, imb_type=imb_type, imb_factor=imb_factor)
            val_dataset = test_dataset
        else: 
            dataset = DirichletImbalanceCIFAR10(data_dir, train=True, download=True, transform=train_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
            train_dataset = DirichletImbalanceCIFAR10(data_dir, train=False, download=True, transform= TwoCropsTransform(train_trsfm), imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
            val_dataset = DirichletImbalanceCIFAR10(data_dir, train=False, download=True, transform=test_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
...
class LogitAdjustExpertLoss(nn.Module):
    def __init__(self, weights, cls_num_list, power=1.0):
        super().__init__()
        
        self.cls_num_list = cls_num_list
        
        # prior should be sorted
...
def main(config, args):
    ...
    if config['data_loader']['type'] == "ImbalanceCIFAR100DataLoader":
        many_shot = train_cls_num_list > 100
        few_shot = train_cls_num_list < 20
        medium_shot = ~many_shot & ~few_shot
    elif config['data_loader']['type'] == "ImbalanceCIFAR10DataLoader":
        many_shot = train_cls_num_list > 1000
        few_shot = train_cls_num_list < 200
        medium_shot = ~many_shot & ~few_shot
    else:
    ...
    for i in range(trials):
        ...
        if config['data_loader']['type'] == "ImbalanceCIFAR100DataLoader":
            data_loader = DirichletImbalanceCIFAR100DataLoader(
                config['data_loader']['args']['data_dir'],
                batch_size=128,
                shuffle=False,
                training=False,
            ...
            data_loader = DirichletImbalanceCIFAR10DataLoader(
                config['data_loader']['args']['data_dir'],
                batch_size=128,
                shuffle=False,
                training=False,
...
def test_validation(data_loader, model, num_classes, aggregation_weight, device, many_shot, medium_shot, few_shot):
    ...
    print("Many-shot {0:.2f}, Medium-shot {1:.2f}, Few-shot {2:.2f}, Mean {3:.2f}, All {4:.2f}".format(many_shot_acc * 100, medium_shot_acc * 100, few_shot_acc * 100, mean_acc * 100, eval_acc_mic_top1* 100))
         
    return np.round(many_shot_acc * 100, decimals=2), np.round(medium_shot_acc * 100, decimals=2), np.round(few_shot_acc * 100, decimals=2), np.round(mean_acc * 100, decimals=2) ,np.round(eval_acc_mic_top1 * 100, decimals=2)
...
if __name__ == '__main__':
    ...
    args.add_argument('--epochs', default=1, type=int,
                      help='indices of GPUs to enable (default: all)')
    args.add_argument('--ir', default=0.01, type=float)
