...
from data_loader.imbalance_cifar import IMBALANCECIFAR100, IMBALANCECIFAR10
...
class TestAgnosticImbalanceCIFAR10DataLoader(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.01, test_imb_factor=0, reverse=False):
        ...
        if training:
            ...
            dataset = IMBALANCECIFAR10(data_dir, train=True, download=True, transform=train_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
            train_dataset = IMBALANCECIFAR10(data_dir, train=False, download=True, transform= TwoCropsTransform(train_trsfm), imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
            val_dataset = IMBALANCECIFAR10(data_dir, train=False, download=True, transform=test_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse) 
...
def main(config, args):
    ...
    distrb = {
        'uniform': (1,False),
        'forward100': (0.01, False),
        'forward50': (0.02, False),
        'forward25': (0.04, False), 
        'forward10':(0.1, False),
        'forward5': (0.2, False),
        'forward2': (0.5, False),
        'backward100': (0.01, True),
    ...
    test_distribution_set = ["forward100", "forward50",  "forward25", "forward10", "forward5", "forward2", "uniform",  "backward2", "backward5", "backward10", "backward25", "backward50", "backward100"] 
