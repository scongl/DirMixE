...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, data_loader,
                 valid_data_loader=None, lr_scheduler=None, len_epoch=None, eta=20):
        ...
        few_shot_classes = None
        
        if hasattr(self.data_loader, "get_few_shot_classes"):
            few_shot_classes = self.data_loader.get_few_shot_classes(eta)
        ...
        if type(self.criterion).__name__ == "LogitAdjustExpertLoss":
            self.criterion.generate_weights_pool(batch_num)
            self.criterion.set_few_shot_classes(few_shot_classes)

    def _train_epoch(self, epoch, total_epochs=200):
        """
        ...
        # DRW for CE loss
        if type(self.criterion).__name__ == "WeightedExpertLoss":
            self.criterion.generate_weights(epoch, total_epochs)
        elif type(self.criterion).__name__ == "LogitAdjustExpertLoss":
            self.criterion.start_new_epoch(epoch, total_epochs)
